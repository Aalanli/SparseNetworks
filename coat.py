...
class ParallelBlock(nn.Module):
    ...
    def upsample(self, x, size, new_size):
        """ Feature map up-sampling. """
        return self.interpolate(x, new_size=new_size, size=size)

    def downsample(self, x, size, new_size):
        """ Feature map down-sampling. """
        return self.interpolate(x, new_size=new_size, size=size)

    def interpolate(self, x, size, new_size):
        ...
        img_tokens = F.interpolate(img_tokens, size=new_size, mode='bilinear')
    ...
    def forward(self, x1, x2, x3, x4, sizes):
        ...
        upsample3_2 = self.upsample(cur3, size=(H3,W3), new_size=(H2,W2))
        upsample4_3 = self.upsample(cur4, size=(H4,W4), new_size=(H3,W3))
        upsample4_2 = self.upsample(cur4, size=(H4,W4), new_size=(H2,W2))
        downsample2_3 = self.downsample(cur2, size=(H2,W2), new_size=(H3,W3))
        downsample3_4 = self.downsample(cur3, size=(H3,W3), new_size=(H4,W4))
        downsample2_4 = self.downsample(cur2, size=(H2,W2), new_size=(H4,W4))
